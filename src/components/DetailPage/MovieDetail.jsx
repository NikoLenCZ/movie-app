...
const MovieDetail = () => {
  ...
  const getMovieDetail = async () => {
    ...
    try {
      ...
      const responses = await Promise.all([fetch(movieUrl), fetch(castUrl)]);
  ...
  return (
    <div className='movieDetail'>
      ...
      <div className='detailBox'>
        <img
          src={movieDetail.backdrop_path ? `https://image.tmdb.org/t/p/w500${movieDetail.backdrop_path}` : '/img/movie.png'}
          alt='movie poster'
        />
        <div className='detailDesc'>
          ...
          <p>
            Release date: {movieDay}. {movieMonth}. {movieYear}
          </p>
          <p>Original language: {movieDetail.original_language}</p>
          <p>Runtime: {movieDetail.runtime} min</p>
          <p>
            Homepage: <a href={movieDetail.homepage}>{movieDetail.homepage ? movieDetail.title : ''}</a>
          </p>
        </div>
      </div>
        ...
        {actorsToShow
          ...
          .map((actor) => (
            <li key={actor.id}>
              <div className='actor'>
                <img
                  src={actor.profile_path ? `https://image.tmdb.org/t/p/w500${actor.profile_path}` : '/img/actor.png'}
